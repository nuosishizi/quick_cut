...
- 逐词高亮必须走 `StyledTextCLS`，关键帧 `SetKeyFrames(kf, true)`，**只写当前词**的填充色（Index 0）。词结束必须写空关键帧，停顿全灭，禁止把高亮拖到下一词。给每个词都写区间会把 Element 4 整句底切成一词一块。禁止 `AddTool("CharacterLevelStyling")`。高亮写完后必须再锁一次 `Level4=0`。
